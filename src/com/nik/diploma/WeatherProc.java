...
public class WeatherProc {
    ...
    public static void mane() throws Exception{
        ...
        File stationsData = new File("/usr/local/diploma/stations_hourly_weather/stats.csv");

        CSVParser collParser = CSVParser.parse(collData, Charset.forName("UTF-8"), CSVFormat.RFC4180);
        CSVParser stationsParser = CSVParser.parse(stationsData, Charset.forName("UTF-8"), CSVFormat.TDF);
        PrintWriter writer = new PrintWriter("full.csv", "UTF-8");

        for (CSVRecord csvRecord : stationsParser) {
            Station station = new Station();
            station.src_id = (csvRecord.get(0));
            station.lat = (csvRecord.get(6));
            station.lon = (csvRecord.get(7));
